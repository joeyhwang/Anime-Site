...
import React, {
  ...
  export async function getServerSideProps(context: { query: { query: string; }; }) {
    ...
    const res = await fetch(
        `https://api.myanimelist.net/v2/anime?q=${query}&limit=50&fields=id,title,main_picture,start_date,end_date,synopsis,mean,rank,popularity,num_list_users,num_scoring_users,media_type,status,genres,num_episodes,start_season,broadcast,source,average_episode_duration,rating,background,studios`,
      {
        method: 'get',
